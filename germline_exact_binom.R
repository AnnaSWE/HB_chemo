...
# exact binomial test
# Original code by Tim Coorens - January 2019
# Minor modifications by Anna Wenger
#-------------------------------------------------
...
exact.binomial=function(gender,NV,NR,cutoff=-5){
  ...
  # variant calls of multiple normal samples from same individual (aggregate coverage
  ...
  #if only one sample
  if(is.integer(NR)==TRUE){
    XY_chromosomal = grepl("X|Y",names(NR))
    autosomal = !XY_chromosomal
  } else { #If several samples
  ...
  if(gender=="female"){
    ...
    if(is.integer(NR)==TRUE){ #if only one sample (ie NR is an integer instead of dataframe) then just set those values as NV_vec and NR_vec resp (cannot do rowSums on an integer, will give error)
      NV_vec=NV
      NR_vec=NR
    } else { #If several samples, do the rowSums function
      NV_vec = rowSums(NV)
      NR_vec = rowSums(NR)
    }
  ...
  if(gender=="male"){
    ...
    # Add a test to make sure there are any autosomal variants (will throw an error in binom test otherwise) /AW
    if (sum(autosomal)>0) {
    ...
    if(is.integer(NR)==TRUE){ #if only one sample (ie NR is an integer instead of dataframe) then just set those values as NV_vec and NR_vec resp (cannot do rowSums on an integer, will give error)
      NV_vec = NV[XY_chromosomal]
      NR_vec = NR[XY_chromosomal]
    } else { #If several samples, do the rowSums function
    ...
    if (sum(XY_chromosomal)>0) {
      for (n in 1:sum(XY_chromosomal)){ # Add a test to make sure there are any XY chromosomal variants (will throw an error in binom test otherwise /AW
        if(NR_vec[n]>0){   
