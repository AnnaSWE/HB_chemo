...
# https://github.com/MaximilianStammnitz/Indelwald/tree/master

##################################################################

# Original code by Max Stammnitz.
# Modified by Anna Wenger 7th of May 2024

# Modifications only included the change of how to read in the
# the reference fasta file
# His original approach was:
...
# This didn't work so changed it to a GRanges approach
# (which is what we use for plotting SBS spectra) seen in the code
# below. Also changed the naming from reference to genomeFile.

##################################################################

# Example usage of code:
# genomeFile="/lustre/scratch124/casm/team78pipelines/canpipe/live/ref/human/GRCh38_full_analysis_set_plus_decoy_hla/genome.fa"
# NOTE: change to Ch38 if apprpriate!
# input.vcf <- read.vcfR("/lustre/scratch126/casm/team274sb/aw35/hepatoblastoma/NanoSeq/indels/03_DNA_analyses/SigProfilerMatrix/01_Input/PD57225aa_ds0001.indels.pass.vcf")
# input.vcf <- cbind(input.vcf@fix[,c('CHROM', 'POS', 'REF', 'ALT')], 'TRIPLET'= NA)
# spectrum.out <- indel.spectrum(x = input.vcf)
...
indel.spectrum <- function(x){
  ...
  if(nrow(dels.1bp) > 0){
    ...
    dels.1bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(dels.1bp[,'CHROM']), IRanges(as.numeric(dels.1bp[,'POS']) - 9, 
                                                                                                    as.numeric(dels.1bp[,'POS']) + 11))))
    
    #cbind(dels.1bp,rep(NA,nrow(dels.1bp)))
    ...
    dels.1bp.context.rc <- as.character(reverseComplement(scanFa(genomeFile, GRanges(as.character(dels.1bp[,'CHROM']), 
                                                                                     IRanges(as.numeric(dels.1bp[,'POS']) - 9, 
                                                                                    as.numeric(dels.1bp[,'POS']) + 11)))))
  ...
  if(nrow(ins.1bp) > 0){
    ...
    ## extract 10 bp upstream/downstream sequence context from reference
    ins.1bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(ins.1bp[,'CHROM']), IRanges(as.numeric(ins.1bp[,'POS']) - 9, 
    ...
    ## need the central base to be pyrimidine-centred, i.e. C or T, hence also run reverseComplements on full contexts
    ins.1bp.context.rc <- as.character(reverseComplement(scanFa(genomeFile, GRanges(as.character(ins.1bp[,'CHROM']), 
                                                                                     IRanges(as.numeric(ins.1bp[,'POS']) - 9, 
                                                                                             as.numeric(ins.1bp[,'POS']) + 10)))))
  ...
  if(nrow(dels.2bp) > 0){
    ...
    dels.2bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(dels.2bp[,'CHROM']), IRanges(as.numeric(dels.2bp[,'POS']) - 9, 
  ...
  if(nrow(dels.3bp) > 0){
    ...
    dels.3bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(dels.3bp[,'CHROM']), IRanges(as.numeric(dels.3bp[,'POS']) - 14, 
  ...
  if(nrow(dels.4bp) > 0){
    ...
    dels.4bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(dels.4bp[,'CHROM']), IRanges(as.numeric(dels.4bp[,'POS']) - 19, 
  ...
  if(nrow(dels.5bp) > 0){
    ...
    dels.5bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(dels.5bp[,'CHROM']), IRanges(as.numeric(dels.5bp[,'POS']) - 499, 
                                                                                                    as.numeric(dels.5bp[,'POS']) + 5 + 500))))
  ...
  if(nrow(ins.2bp) > 0){
    ...
    ins.2bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(ins.2bp[,'CHROM']), IRanges(as.numeric(ins.2bp[,'POS']) - 9, 
  ...
  if(nrow(ins.3bp) > 0){
    ...
    ins.3bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(ins.3bp[,'CHROM']), IRanges(as.numeric(ins.3bp[,'POS']) - 14, 
  ...
  if(nrow(ins.4bp) > 0){
    ...
    ins.4bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(ins.4bp[,'CHROM']), IRanges(as.numeric(ins.4bp[,'POS']) - 19, 
  ...
  if(nrow(ins.5bp) > 0){
    ...
    ins.5bp.context=as.vector(scanFa(genomeFile, GRanges(as.character(ins.5bp[,'CHROM']), IRanges(as.numeric(ins.5bp[,'POS']) - 499, 
                                                                                                  as.numeric(ins.5bp[,'POS']) + 500))))
